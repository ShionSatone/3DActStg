...
void CCamera::Update(void)
{
	...
	if (CManager::GetMode() == CScene::MODE_GAME && m_bBoss == false)
	{//ゲーム画面のとき
		...
		if (pGame->GetGameMode() == pGame->GAMEMODE_BOSS && m_posR.y < -100.0f && pPlayer->GetPosition().x >= 10000.0f && m_bProject == false)
		{
			m_bProject = true;
		}
		else if (pGame->GetGameMode() == pGame->GAMEMODE_BOSS && m_posR.y < -4700.0f && pPlayer->GetPosition().x >= 10452.0f)
		{
			m_bBoss = true;		//ボス戦状態にする
		}
		else if (pGame->GetGameMode() == pGame->GAMEMODE_BOSS && m_posR.y >= -4780.0f)
		{//ボス戦になったら
			m_posR.y = pPlayer->GetPosition().y;
			m_posV.y = pPlayer->GetPosition().y;
		}
	}
	else if (CManager::GetMode() == CScene::MODE_GAME && m_bBoss == true)
	{
		if (pGame->GetGameMode() != pGame->GAMEMODE_BOSS)
		{//ゲームモードがボスじゃないとき
...
void CCamera::Control()
{
	...
	//case MODE_TITLE:
	//	//目的の視点を設定
	//	m_posVDest.x = pPlayer->pos.x - 100.0f + sinf(m_rot.y + D3DX_PI + 0.3f) * LENGRH;
	//	m_posVDest.z = pPlayer->pos.z - 100.0f + cosf(m_rot.y + D3DX_PI + 0.3f) * LENGRH;
	//	break;

	//case MODE_TUTORIAL:
	//	//目的の視点を設定
	//	m_posVDest.x = pPlayer->pos.x + sinf(m_rot.y + D3DX_PI) * LENGRH;
	//	m_posVDest.z = pPlayer->pos.z + cosf(m_rot.y + D3DX_PI) * LENGRH;
	//	break;
	...
	if (pPlayer->GetType() == CPlayerModel::PLAYERTYPE_GOLDFISH)
	{//金魚の時
	...
	//if (pPlayer->GetType() == pPlayer->PLAYERTYPE_HUMAN)
	//{//人間の時

	//	m_bProject = true;		//並行投影
	//}
	///*else */if (pPlayer->GetType() == pPlayer->PLAYERTYPE_GOLDFISH)
	//{//金魚の時

	//	m_bProject = false;		//透視投影
	//}
